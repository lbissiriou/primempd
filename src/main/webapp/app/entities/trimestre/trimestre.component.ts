import { Component, OnInit, OnDestroy } from '@angular/core';
import { HttpErrorResponse, HttpResponse } from '@angular/common/http';
import { Subscription } from 'rxjs';
import { JhiEventManager, JhiAlertService } from 'ng-jhipster';

import { ITrimestre } from 'app/shared/model/trimestre.model';
import { Principal } from 'app/core';
import { TrimestreService } from './trimestre.service';

@Component({
    selector: 'jhi-trimestre',
    templateUrl: './trimestre.component.html'
})
export class TrimestreComponent implements OnInit, OnDestroy {
    trimestres: ITrimestre[];
    currentAccount: any;
    eventSubscriber: Subscription;

    constructor(
        private trimestreService: TrimestreService,
        private jhiAlertService: JhiAlertService,
        private eventManager: JhiEventManager,
        private principal: Principal
    ) {}

    loadAll() {
        this.trimestreService.query().subscribe(
            (res: HttpResponse<ITrimestre[]>) => {
                this.trimestres = res.body;
            },
            (res: HttpErrorResponse) => this.onError(res.message)
        );
    }

    ngOnInit() {
        this.loadAll();
        this.principal.identity().then(account => {
            this.currentAccount = account;
        });
        this.registerChangeInTrimestres();
    }

    ngOnDestroy() {
        this.eventManager.destroy(this.eventSubscriber);
    }

    trackId(index: number, item: ITrimestre) {
        return item.id;
    }

    registerChangeInTrimestres() {
        this.eventSubscriber = this.eventManager.subscribe('trimestreListModification', response => this.loadAll());
    }

    private onError(errorMessage: string) {
        this.jhiAlertService.error(errorMessage, null, null);
    }
}
